...
2. I wrote this CSS for my personalized toggle switch because I didn't want to import [Bootstrap]("https://bootstrap.com/") like in [this implementation]().
...
***
I wrote my own implementation of the dark switch and saved it in `assets/js/theme-switcher.js`. This script caches data in the browser's `localStorage` so that the theme is retained every time the site is reloaded.  It checks the browser's `localStorage` every time the site is reloaded and applies the theme that's written there. It also writes to `localStorage` every time the switch is toggled.
```jsx
function changeTheme(){
	const theme_switch = document.getElementById("theme-switch");
	if (theme_switch.checked) {
...
1. Inserted this in the `HTML.head` to solve [Issue # 2](#issue2) above.
	```html
	<head>
...
2. Finally, inserted this `html` for the toggle switch in the `.nav-bar` of my site's html body. The `js` below the toggle switch ensures that the checkbox always reflects the theme as written in `localStorage` even when the site is reloaded.
	```html
